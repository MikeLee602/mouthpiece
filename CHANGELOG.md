...
**记忆词**：识别错的词一键教会 AI，下次自动改对。

- **菜单栏快捷录入**：popover 顶部按钮「+ 添加词典（记住这个词）」，
  一键弹表单，「识别为 → 应该是」两个输入框，支持从剪贴板贴入 pattern
- **纠错学词**：历史列表每条右边加 ✏️ 编辑按钮，改字后 diff 自动识别
  出的规则（如「王梦松→王孟松」），一键勾选入词典
- **Polish prompt 注入词典**：调 DeepSeek 前把所有词典规则作为
  vocab hint 拼进 system prompt，LLM 见到同音字时优先按用户认可的写法
- 3 处改进联动：手动加规则 / 纠错学习 / AI 润色 都会共享词典

技术：
- `Core/Storage/DiffLearner.swift`：基于 LCS 的字符级 diff，过滤只在
  标点/空格的差异 + 过长的重写
- `Polishing` 协议加 `vocabHint` 参数（默认 [] 向后兼容）
- 测试 93 → 102
